...
executeOnCaeStartup()
import os
filePath ="D:/WL/AbaqusWork/6FB_AutoModel_DDPM/"#csv文件路径
fr = open(filePath+"parameter_sampling_6FB_2D_D25.csv",'r')#csv文件名
...
for i in range(75,100):
    ...
    compensation_coeff_A = float(paralist[i + 1][2])
    compensation_coeff_B = float(paralist[i+1][3])
    v_pd = float(paralist[i+1][4])
    A_bg = float(paralist[i+1][5])
    gap_bd = float(paralist[i+1][6])
    gap_gd = float(paralist[i+1][7])
    f_bd = float(paralist[i+1][8])
    ...
    num_node_section=int(np.ceil(64*np.sqrt(D/R)))  #0.5*np.pi*R/(np.pi*D/n)*n=2048
    l_mesh=2*np.pi*r_out/num_node_section
    l_length=0.5*np.pi*R+20+A_bg+v_pd+alpha0*R
    l_guide=l_length-20-A_bg+D
    num_node_section_bd=36
    delta=R*(1-np.cos(alpha0))
    alpha=alpha0*compensation_coeff_B
    ...
    mdb.models['Model-1'].ExplicitDynamicsStep(name='Step-2', previous='Step-1',
        timePeriod=(l_guide-D)/v_pd-1, massScaling=((SEMI_AUTOMATIC, MODEL, AT_BEGINNING, 1000.0,
    ...
    mdb.models['Model-1'].boundaryConditions['BC-bd'].setValuesInStep(
        stepName='Step-2', u1=0.0, ur2=0.0, amplitude=FREED)
    ...
    mdb.Job(name='Job-6FB-2D-'+ str(i), model='Model-1-' + str(i), description='', type=ANALYSIS,
...
#mdb.saveAs(pathName='D:/WL\AbaqusWork/AbaqusFile-FB-WL/cae-6FB-2D-D25/cae'+paralist[i+1][0])#mdb文件保存路径及文件名
