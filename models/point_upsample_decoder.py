...
class PointUpsampleDecoder(nn.Module):
    def __init__(self, config, in_dim, apply_kl_regularization=False):
        ...
        self.MLP_Res=MLP_Res(in_dim=3, hidden_dim=None, out_dim=3)
    ...
    def upsample_points(self, final_feature, new_xyz):
        ...
        if new_xyz.shape[1] in [4,8,16,32]:
            transfer_point=(self.MLP_Res(new_xyz.transpose(1,2).contiguous())).transpose(1,2).contiguous()
            new_xyz=transfer_point
        else:
            transfer_point=None

        splitted_points = self.fc_layer( torch.cat([final_feature, new_xyz], dim=2).transpose(1, 2) )
        #print(final_feature.shape,new_xyz.shape,splitted_points.shape)
        ...
        return upsampled_points,transfer_point
    ...
    def forward(self, xyz, features, new_xyz, ts=None, label=None, sample_posterior=True):
        ...
        upsampled_points,transfer_point = self.upsample_points(final_feature, new_xyz)
        if self.apply_kl_regularization:
            return final_feature, upsampled_points, kl_loss,transfer_point
        else:
            return final_feature, upsampled_points#,transfer_point