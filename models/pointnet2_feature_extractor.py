...
class PointNet2Encoder(PointNet2SemSegSSG):
    ...
    def _build_model(self):
        ...
        if self.attach_position_to_input_feature:#True
            in_fea_dim = in_fea_dim + 3#0+3
        
        self.use_position_encoding = self.hparams.get('use_position_encoding', False)#False
    ...
    def forward(self, pointcloud, ts=None, label=None):
        ...
        with torch.no_grad():
            if self.use_position_encoding:#False
            ...
            if self.attach_position_to_input_feature:
                xyz_ori = pointcloud[:,:,0:3] / self.scale_factor
                pointcloud = torch.cat([pointcloud, xyz_ori], dim=2)#(B,N,9)
        ...
        if (not label is None) and self.hparams['include_class_condition']:
            ...
            class_emb = self.class_emb(label) # shape (B, condition_emb_dim)(B,128)

        else:
            class_emb = None

        if self.include_global_feature:#False
            if in_fea_dim > 0:
        ...
        #print('l_features',len(l_features))
